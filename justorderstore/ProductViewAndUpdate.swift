//
//  ProductViewAndUpdate.swift
//  justorderstore
//
//  Created by RayRainier on 7/20/17.
...
class UpdateJitterButtonX: UIButton, Jitterable {
    
}

class ProductUpdateControllerX: FormViewController, UINavigationControllerDelegate, Jitterable {
    ...
    let rowActiveSwitch: SwitchRow = SwitchRow("ActiveSwitch") { row in      // initializer
        //row.title = "Not Available"
        }.onChange { row in
            row.title = (row.value ?? false) ? "Not Available" : "Available"
            row.updateCell()
        }.cellSetup { (cell, row) in
            cell.backgroundColor = UIColor.gray.withAlphaComponent(0.9)
    	}

	let rowProductLabel: TextRow = TextRow("ProductLabel") { row in
    ...
    let rowProductSubLabel: TextRow = TextRow("ProductSubLabel") { row in
        	row.title = "Product Sub-Title"
        	row.placeholder = "Enter sub-title here"
        }.cellSetup { (cell, row) in
            cell.backgroundColor = UIColor.gray.withAlphaComponent(0.9)
    	}

    let rowProductDescription: TextAreaRow = TextAreaRow("ProductDesc") {
    		$0.placeholder = "Description"
    		$0.textAreaHeight = .dynamic(initialTextViewHeight: 110)
    	}
    
//    func cellSetupColor()-> Cell {
//        let cell = Cell()
//        	cell.backgroundColor = UIColor.gray.withAlphaComponent(0.9)
...
//    }
    ...
    let productUpdateButton: UpdateJitterButtonX = {
        
        let button = UpdateJitterButtonX(type: .system)
...
//        setupPWActiveSwitch()
    	...
    	tableView?.backgroundColor = UIColor.gray.withAlphaComponent(0.9)
